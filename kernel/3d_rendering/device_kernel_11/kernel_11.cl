...
__kernel void default_function(__global coord_t* restrict triangle_3d, 
                               __global int* restrict angle,
  ...
  coord_t triangle_3d__[3191];
  for (int x11 = 0; x11 < 3191; ++x11) {
    triangle_3d__[x11] = triangle_3d[x11];
  }

  int angle_[1];
  for (int x12 = 0; x12 < 1; ++x12) {
    angle_[x12] = angle[x12];
  }
  ...
  for (int m = 0; m < 3191; ++m) {
    ...
    */
    ...
    for (int x1 = 0; x1 < 9; ++x1) {
      ...
      triangle_3d_[x1] = triangle_3d__[m].val[x1];
    }
    int fragment[2000];
    pack_t pixels[500];
    int triangle_2d[7];
